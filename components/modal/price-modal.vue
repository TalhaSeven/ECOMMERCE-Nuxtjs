...
watch(
    ...
    async (newValue, oldValue) => {
        await axios.get(`http://localhost:3050/api/v1/price/${newValue.id}`).then((res) => {
...
const submit = () => {
    ...
    axios.post('http://localhost:3050/api/v1/price', form.value).then((res) => {
        Swal.fire({
            title: 'Good save!',
...
<template>
    <dialog id="my_modal_1" class="modal" :class="{ 'modal-open': props.modalOpen }">
        <div class="modal-box">
            ...
            <form @submit.prevent="submit" class="space-y-8">
                <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-3 lg:grid-cols-3 xl:gap-x-8">
                    <div>
                        ...
                        <input type="number" id="price"
                            ...
                            placeholder="Stock Code" required v-model="form.price">
                    </div>
                    <div>
                        <label for="discountPrice"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Discount Price</label>
                        <input type="number" id="discountPrice"
                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light"
                            @change="handleDiscountPriceChanged"
                            placeholder="Stock Code" required v-model="form.discountPrice">
                    </div>
                    <div>
                        <label for="discountRate"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Discount Rate</label>
                        <input type="number" id="discountRate"
                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light"
                            @change="handleDiscountRateChanged"
                            placeholder="Stock Code" required v-model="form.discountRate">
                    </div>
                </div>
                <div class="modal-action">
                    <button type="submit"
                        ...
                        Save Price</button>
                    <button type="button" class="btn" @click="handleModalClose()">Close</button>
                </div>
            </form>
        </div>
    </dialog>
